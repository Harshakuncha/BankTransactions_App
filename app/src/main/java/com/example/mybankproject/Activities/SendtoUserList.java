...
import Adapters.SendtoUserAdapter;
import data.user;
...
public class SendtoUserList  extends AppCompatActivity implements SendtoUserAdapter.OnUserListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sendto_user_list);
    ...
    public void onUserClick(int position) {
        ...
        new transactiondphelper(this).insertTransferData(fromUserAccountName, toUserAccountName, transferAmount, 1);
        Toast.makeText(this, "Transaction Successful!!", Toast.LENGTH_LONG).show();

        startActivity(new Intent(SendtoUserList.this, HomeActivity.class));
        finish();
    }
        ...
        builder_exitButton.setTitle("Do you want to cancel the transaction?").setCancelable(false)
                .setPositiveButton ("yes", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick (DialogInterface dialogInterface, int i) {
                      ...
                      transactiondphelper dbHelper = new transactiondphelper(getApplicationContext());
                        SQLiteDatabase db = dbHelper.getWritableDatabase();
                        ContentValues values = new ContentValues();

                        values.put(transactiondb.TransactionEntry.COLUMN_FROM_NAME, fromUserAccountName);
                        values.put(transactiondb.TransactionEntry.COLUMN_TO_NAME, toUserAccountName);
                        values.put(transactiondb.TransactionEntry.COLUMN_STATUS, 0);
                        values.put(transactiondb.TransactionEntry.COLUMN_AMOUNT, transferAmount);

                        db.insert(transactiondb.TransactionEntry.TABLE_NAME, null, values);

                        Toast.makeText(SendtoUserList.this, "Transaction Cancelled!", Toast.LENGTH_LONG).show();
                        startActivity(new Intent(SendtoUserList.this, user.class));
    ...
    private void displayDatabaseInfo() {
        ...
        String[] projection = {
                userdb.UserEntry.COLUMN_USER_NAME,
                userdb.UserEntry.COLUMN_USER_ACCOUNT_BALANCE,
                userdb.UserEntry.COLUMN_USER_ACCOUNT_NUMBER,
                userdb.UserEntry.COLUMN_USER_PHONE_NO,
                userdb.UserEntry.COLUMN_USER_EMAIL,
                userdb.UserEntry.COLUMN_USER_IFSC_CODE,
        };

        Cursor cursor = db.query(
                userdb.UserEntry.TABLE_NAME,   // The table to query
